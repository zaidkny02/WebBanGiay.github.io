...
function showProduct() {
    if (lstProduct) {
        ...
        for (var i = 0; i < lstProduct.data.length; i++) {
            ...
            str += `<div class='product' id="prod${i}">
        ...
        if(Product.data[i].prod_id == "id"+id)
        {
            ...
            for (var i = 0; i < ProductInCart.data.length; i++)
            {
                if(ProductInCart.data[i].prod_id == "id"+id)
                {
                    ...
                    var sl = parseInt(ProductInCart.data[i].prod_sl) + 1;
                    console.log(sl);
                    ProductInCart.update("id"+id,sl);
...
function search(){
    ...
    if(parseInt(min) || parseInt(max))
    {
        for (var i = 0; i < Product.data.length; i++) {
            ...
            if(parseInt(Product.data[i].prod_price) < parseInt(min) || parseInt(Product.data[i].prod_price) > parseInt(max))
            {   
                var id = "prod" + i;
    ...
    {
        alert('Vui lòng nhập đúng giá cả');
    }
}
/*var regex = /.*ger.*/
...
function loadProduct() {
    ...
    if (myJSON == null) {
        var i = 0;
        for (i = 0; i < 5; i++) 
            Product.add("id" + i, "Giày Cao Gót" + i, (250000+i*10000).toString(), "img/"+(i+1)+".jpg","Giày cao gót");
        for (i = 5; i < 10; i++) 
